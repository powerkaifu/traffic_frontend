 ...
 * ====================================================================
 * 
 * 基於 VLRJM60、VLRJX00、VLRJX20 三個VD偵測器的真實數據（2024/12-2025/05）
 * 
 * 核心原則：
 * 1. 視覺車流量 ≈ 傳送給後端的特徵數據
 * 2. 所有特徵值必須在VD訓練數據範圍內
 * 3. 讓評審看到「車多 = 預測秒數長」的對應關係
 * 
 * ====================================================================
 * 📊 VD 真實數據範圍摘要
 * ====================================================================
...
/**
 * 將流量轉換為每秒車輛數，再計算生成間隔
 * @param {number} volumePer5Min - 5分鐘內的車輛數
 * @returns {number} 生成間隔（毫秒）
 */
function volumeToInterval(volumePer5Min) {
  if (volumePer5Min === 0) return 30000 // 無車輛時，30秒一台
  const vehiclesPerSecond = volumePer5Min / 300 // 5分鐘 = 300秒
  const intervalSeconds = 1 / vehiclesPerSecond
 ...
 * 1. 生成間隔計算：
 *    - 基於「每5分鐘車輛數」反推生成間隔
 *    - 公式：interval = 300秒 / 目標車輛數 * 1000ms
 *    - 例如：目標10輛/5分鐘 → 300/10 = 30秒/輛 = 30000ms
 * 
 * 2. peakMultiplier（流量強度倍率）：
 *    - 實際作用：interval / peakMultiplier = 實際間隔
 *    - 數值越大，實際間隔越短，車流越密集
 *    - 範圍：0.5-4.0（基於VD數據的流量變化範圍）
 * 
 * 3. 車型權重（對應VD資料的車種分布）：
 *    - motor (機車)：根據時段30-80%
 *    - small (小客車)：根據時段15-55%
 *    - large (大客車)：固定10-15%（VD資料中大客車較少）
 * 
 * 4. 目標數據範圍（確保在VD訓練範圍內）：
 *    - 尖峰：Volume 8-15/5分鐘，Occupancy 40-60%，Speed 20-35 km/h
 *    - 離峰：Volume 4-8/5分鐘，Occupancy 15-30%，Speed 30-45 km/h
 *    - 凌晨：Volume 1-3/5分鐘，Occupancy 5-15%，Speed 35-55 km/h
 */
...
export const vdBasedTimeScenarios = [
  ...
  // ========================================
  {
    ...
    targetFeatures: {
      ...
      // 各車種流量（基於VD數據凌晨時段推估）
      volumeByType: {
        motor: 1,      // 機車：1輛/5分鐘（凌晨機車最多）
        small: 1,      // 小客車：1輛/5分鐘
        large: 0,      // 大客車：0輛/5分鐘（凌晨很少）
      },
