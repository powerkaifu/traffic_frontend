...
export const GREEN_LIGHT_PREDICTION_CONFIG = {
   ...
   *
   * 調整策略已改為：在生成層控制數據，不在 API 層調整
   * - 1.0: 不調整，直接發送原始生成數據（推薦）
   * - 0.75 及以下：已淘汰（後端會飽和到 99 秒）
   *
   * @type {number}
   * @default 1.0
   */
  PEAK_HOUR_VOLUME_ADJUSTMENT: 1.0,

  /**
   * 離峰時段（09:00-17:00,19:00-23:00）流量調整係數
   *
   * 調整策略已改為：在生成層控制數據，不在 API 層調整
   * - 1.0: 不調整，直接發送原始生成數據（推薦）
   *
   ...
   * @default 1.0
   */
  OFF_PEAK_HOUR_VOLUME_ADJUSTMENT: 1.0,
   ...
   * @default 1.0
   */
  NIGHT_HOUR_VOLUME_ADJUSTMENT: 1.0,
