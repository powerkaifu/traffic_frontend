...
export const GREEN_LIGHT_PREDICTION_CONFIG = {
   ...
   *
   * 預期效果：
   * - 0.65: 流量 ↓35% → 預測時間約 ↓35% (99秒 → ~64秒)
   * - 0.70: 流量 ↓30% → 預測時間約 ↓30% (99秒 → ~69秒)
   * - 0.75: 流量 ↓25% → 預測時間約 ↓25% (99秒 → ~74秒) ⭐ 推薦（達到 65-85 秒目標）
   * - 0.80: 流量 ↓20% → 預測時間約 ↓20% (99秒 → ~79秒)
   * - 0.85: 流量 ↓15% → 預測時間約 ↓15% (99秒 → ~84秒)
   *
   * @type {number}
   * @default 0.75
   */
  PEAK_HOUR_VOLUME_ADJUSTMENT: 0.75,

  /**
   * 離峰時段（09:00-17:00）流量調整係數
   *
   * 預期效果：
   * - 0.70: 流量 ↓30% → 預測時間約 ↓30% (80秒 → ~56秒)
   * - 0.75: 流量 ↓25% → 預測時間約 ↓25% (80秒 → ~60秒)
   * - 0.80: 流量 ↓20% → 預測時間約 ↓20% (80秒 → ~64秒) ⭐ 推薦（達到 65-85 秒目標）
   * - 0.85: 流量 ↓15% → 預測時間約 ↓15% (80秒 → ~68秒)
   * - 0.90: 流量 ↓10% → 預測時間約 ↓10% (80秒 → ~72秒)
   *
   * @type {number}
   * @default 0.80
   */
  OFF_PEAK_HOUR_VOLUME_ADJUSTMENT: 0.8,

  /**
   * 夜間時段（19:00-07:00）流量調整係數
   *
   * 預期效果：
   * - 0.75: 流量 ↓25% → 預測時間約 ↓25% (60秒 → ~45秒)
   * - 0.80: 流量 ↓20% → 預測時間約 ↓20% (60秒 → ~48秒) ⭐ 推薦（達到 65-85 秒目標）
   * - 0.85: 流量 ↓15% → 預測時間約 ↓15% (60秒 → ~51秒)
   * - 0.90: 流量 ↓10% → 預測時間約 ↓10% (60秒 → ~54秒)
   * - 1.00: 流量不變   → 預測時間不變     (60秒 → 60秒)
   *
   * @type {number}
   * @default 0.80
   */
  NIGHT_HOUR_VOLUME_ADJUSTMENT: 0.8,
