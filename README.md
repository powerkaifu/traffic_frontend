...
- [] 右上角「尖峰、離峰、凌晨」三個為情境按鈕，點擊後須對應不同的車流量，尖峰時段車流量較大，離峰時段車流量較小，凌晨時段車流量最小。
  ...
  - 離峰希望調整到 50~70 秒
  - 凌晨希望調整到 40~45 秒
- [] 切換每日自動模擬，快速模擬每日 24 小時的車流，
  - 調整每個時段，流量強度、生成間隔需搭配起來，才能達到想要的車流量。
- [] 右下角「特徵模擬數據」抓取是否是在南北向倒數十秒時送出，在這期間為抓取數據的時間，而送出後要清空數據，繼續收集四個路口的特徵數據。
  - 目前每個路口抓到幾百輛車子，數據很誇張，推測是車子生成動畫太快，擷取到的車子數量太多。

### IndexPage.vue（十字路口、車輛動畫的頁面）

- [x] 車子到紅燈停止線，要有緩慢減速的動畫效果，不能直接停下來。
- [x] 車子快碰撞時，要有緩慢減速的動畫效果，現在是撞上去，停止一下再前進。
- [x] 東西向車子動畫正常，可以起點->終點(橫跨整個頁面)
- [x] 南下車輛在停紅燈時，排隊到第七輛時，車道會發生車輛直接消失的情況，應該要讓車子持續排隊，不能有消失的情況。
  - 東西向也有此情況，但不明顯。
- [x] 北上車輛在停完紅燈後出發，橫跨十字路口後，在到達終點前，車輛會消失，應該要讓車子持續行駛到超過瀏覽器頂端。
- [] 車子速度需要有變化，大車、小車、摩托車速度最大值不同，機車速度較快，小車次之，大車最慢。
...
### VisualizationPage.vue（視覺化數據的頁面）

---

- [] 車子轉彎功能
  - 搭配 gsap 與其插件 MotionPath，讓車輛可以沿著路徑轉彎。
...
- [] Lumo 小助手能做甚麼事？
- [] 視覺化數據能呈現甚麼？數據動畫需吸睛，讓評審或瀏覽者覺得有趣、很酷。
...
如果 AI agent 進階模型請求次數用完，可以使用 Context7 來詢問問題。
